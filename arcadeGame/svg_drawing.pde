...
void initSvgDrawing() {
  ...
  nullScore = loadShape("pubstick_scorenumber_null.svg");
  ...
  timerSvgNumbers[0] = allSvgTimers[0];
  timerSvgNumbers[1] = allSvgTimers[0];
  timerSvgNumbers[2] = allSvgTimers[0];
}
...
void updateTimerSvgValues(int remainingTime) {
  if (frameCount%30 == 0) {
    char[] currentTime = formattedRemainingTimeForSvg(remainingTime).toCharArray();
...
String formattedRemainingTimeForSvg(int remainingTime) {
  int minute = (remainingTime / (1000*60)) % 60;
  int second = (remainingTime / 1000) % 60;
...
void bottomScoreDisplay(Player[] players) {
...
void setScoreImageToZero() {
  scoreSvgNumbers[0] = nullScore;
  scoreSvgNumbers[1] = allSvgNumbers[0];
}
...
void setBottomScoreImage(int scoreAmount) {
  ...
  // Skip first zero so we don't print '01' for a score of 1
  if (firstDigit == 0) {
    tempscoreSvgNumbers[0] = nullScore;
  } else {
...
void drawPlayerSelectIndicator(int numPlayers) {
